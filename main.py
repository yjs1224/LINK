...
from models import birnn as BiRNN,cnn as CNN,lstmatt as LSTMATT,fcn as FCN, r_bilstm_c as RBC,\
    bilstm_dense as BLSTMDENSE,sesy as SESY, gnn as GNN, KE as KE
...
task_metrics = {"steganalysis" : "accuracy",
                ...
                "KE-steganalysis":"accuracy",
                "KE-graph_steganalysis":"accuracy",
                "ss":"f1_score",
                "gss":"f1_score"}
    ...
    if not Configs.use_plm:
       pass
        ...
        if Configs.model.lower() in ["ts-csw", "cnn"]:
            ...
            model = KE.BERT_TC.from_pretrained(model_name_or_path,
                                                    **{**Model_Configs, "class_num": Configs.class_num, "concept_embedding_path":concept_embedding_path})
        elif Configs.model.lower() in ["gnn"]:
            Model_Configs = Configs.GNN
            model = GNN.BERT_TC.from_pretrained(model_name_or_path,
                                               **{**Model_Configs, "class_num": Configs.class_num, })
        else:
...
def train(model, Configs, tokenizer, kn):
    ...
    if Configs.model.lower() in ["gnn", "cnn", "birnn", "fcn", "lstmatt"] or \
        ...
        kwargs = {"kn":kn}
    train_dataset = load_and_cache_examples(Configs.Dataset, Configs.task_name, tokenizer, overwrite_force=True, seed=Configs.seed, **kwargs)
    ...
    Training_Configs.eval_and_save_steps =  Training_Configs.eval_and_save_steps if  Training_Configs.eval_and_save_steps>0 else len(train_dataloader)
...
def evaluate(model, tokenizer, Configs, task_name, split="dev", prefix="", use_tqdm=True):
    ...
    results = {}
    if task_name == "record":
        eval_dataset, eval_answers = load_and_cache_examples(Configs.Dataset, task_name, tokenizer, split=split, seed=Configs.seed)
    else:
        eval_dataset = load_and_cache_examples(Configs.Dataset, task_name, tokenizer, split=split, seed=Configs.seed)
    ...
    for batch in eval_dataloader:
        ...
        if batch_seq_length < max_seq_length:
            inputs = {"input_ids": batch[0][:, :batch_seq_length].contiguous(),
                      ...
                      "labels": batch[3]}
            if Configs.task_name == "KE-steganalysis" or Configs.task_name == "KE-graph_steganalysis":
                inputs = {**inputs, "concept_ids": batch[4][:, :batch_seq_length].contiguous(),
...
def load_and_cache_examples(Dataset_Configs, task, tokenizer, split="train", overwrite_force=False, seed=42, **kwargs):
    if task == "steganalysis" or  task == "ss":
        from processors.process import SteganalysisProcessor as DataProcessor
    elif task == "graph_steganalysis" or task == "gss":
        from processors.graph_process import GraphSteganalysisProcessor as DataProcessor
    elif task == "KE-steganalysis":
        from processors.KGprocesser import KGSteganalysisProcessor as DataProcessor
    elif task == "KE-graph_steganalysis":
        from processors.KG_Graphprocesser import KG_GraphSteganalysisProcessor as DataProcessor


    # Load data features from cache or dataset file
    cached_tensors_file = os.path.join(
        Dataset_Configs.csv_dir,
        "tensors_{}_seed{}_{}".format(
            split, seed, str(task),
        ),
    )

    if os.path.exists(cached_tensors_file) :
        ...
        ## TODO
        ##  different dataset split implements, NOTICE!
        if task in ["KE-steganalysis", "KE-graph_steganalysis"]:
            processor = DataProcessor(tokenizer,split_ratios=Dataset_Configs.split_ratio, **kwargs)
            processor.init_examples(Dataset_Configs.csv_dir)
        else:
            kwargs["data_dir"] = Dataset_Configs.csv_dir
            processor = DataProcessor(tokenizer, **kwargs)
        for split_tmp in ["train", "dev", "test"]:
            cached_tensors_file = os.path.join(
                Dataset_Configs.csv_dir,
                "tensors_{}_seed{}_{}".format(
                    split_tmp, seed, str(task),
                ),
            )
            if split_tmp == "train":
...
def main(Configs, kn):
    ...
    if Configs.use_processor:
        ...
        if not Dataset_Configs.resplit and os.path.exists(Dataset_Configs.csv_dir) and \
            ...
            train_texts,test_texts, train_labels,test_labels = train_test_split(texts,labels,train_size=1-Dataset_Configs.split_ratio[2])
            train_texts,val_texts, train_labels,val_labels, = train_test_split(train_texts, train_labels, train_size=int(len(labels)*Dataset_Configs.split_ratio[0]))
    ...
    if Configs.use_processor:
        # train_dataset = load_and_cache_examples(Dataset_Configs, Configs.task_name, tokenizer, seed=Configs.seed)  # , evaluate=False)
        global_step, tr_loss = train(model, Configs, tokenizer, kn)
        ...
        for checkpoint in checkpoints:
            ...
            test_macrofscore = result["macro_f1"]
            test_Fscore = result["f1_score"]

        return test_acc, test_macrofscore, test_precision, test_recall, test_Fscore,
...
if __name__ == '__main__':
    ...
    with open(result_path, "w", encoding="utf-8") as f_record:
        for i in range(Configs.get("repeat_num", 1)):
            test_acc, test_macrofscore, test_precision, test_recall, test_Fscore = main(Configs, kn=args.kn)
            f_record.write("test phase:\nseed\t{}\naccuracy\t{:.4f}\nmacro fscore\t{:.4f}\nprecision\t{:.4f}\nrecall\t{:.4f}\nf1_score\t{:.4f}\n"
                    .format(Configs.seed,test_acc * 100,test_macrofscore*100 ,test_precision * 100, test_recall * 100, test_Fscore * 100))
            Configs.seed += 1 ## update random  seed
            total_test_acc.append(test_acc)
            total_test_macrofscore.append(test_macrofscore)
            total_test_precision.append(test_precision)
            total_test_recall.append(test_recall)
            total_test_Fscore.append(test_Fscore)

        message = "\n\nEverage results on {} runs\naccuracy\t{:.2f}%+{:.2f}%\nmacro fscore\t{:.2f}%+{:.2f}%\nprecision\t{:.2f}%+{:.2f}%\nrecall\t{:.2f}%+{:.2f}%\nf1_score\t{:.2f}%+{:.2f}%"\
            .format(Configs.get("repeat_num", 1),
                    np.mean(total_test_acc)*100, np.std(total_test_acc)*100,
                    np.mean(total_test_macrofscore)*100, np.std(total_test_macrofscore)*100,
