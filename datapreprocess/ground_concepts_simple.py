...
if __name__ == "__main__":
    ...
    COVER_FILE = os.path.join(DATA_PATH, "cover.txt")
    STEGO_FILE = os.path.join(DATA_PATH, "stego.txt")

    read_model_vocab(config["paths"]["gpt2_vocab"])

    cover = read_csv(COVER_FILE)
    stego = read_csv(STEGO_FILE)

    if args.mode == 1:
        grounding_sentences(cover, [""]*len(cover), "cover", DATA_PATH)
    grounding_sentences(stego, [""] * len(stego), "stego", DATA_PATH)