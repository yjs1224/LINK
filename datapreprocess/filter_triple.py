...
if __name__ == "__main__":
    ...
    total_concepts = []
    if args.mode ==1 :
        data = read_json(DATA_PATH + '/cover.{}hops_{}_triple.json'.format(T, max_B))
        f_data, f_concepts = filter_directed_triple(data, max_concepts=400, max_triples=1000)
        total_concepts += f_concepts
        save_json(f_data, DATA_PATH + '/cover.kg.json')

    data = read_json(DATA_PATH + '/stego.{}hops_{}_triple.json'.format(T, max_B))
    f_data, f_concepts = filter_directed_triple(data, max_concepts=400, max_triples=1000)
    total_concepts += f_concepts
    save_json(f_data, DATA_PATH + '/stego.kg.json')
