...
def load_total_concepts(data_path):    
    ...
    total_concepts = []
    total_concepts_id = []
    for path in [data_path + "/cover.concepts_nv.json", data_path + "/stego.concepts_nv.json"]:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="hc1")
    parser.add_argument("--mode", type=int, default=0)
    parser.add_argument("--T", type=int, default=2, help="max hop")
    parser.add_argument("--max_B", type=int, default=100)
    args = parser.parse_args()

    T, max_B = args.T, args.max_B
    DATA_PATH = config["paths"][args.dataset + "_dir"]

    load_resources()
    load_cpnet()
    load_total_concepts(DATA_PATH)
    if args.mode==1:
        process(os.path.join(DATA_PATH, "cover.concepts_nv.json"),
                os.path.join(DATA_PATH,"cover.{}hops_{}_triple.json".format(T, max_B)), T, max_B)
    process(os.path.join(DATA_PATH, "stego.concepts_nv.json"),
           os.path.join(DATA_PATH, "stego.{}hops_{}_triple.json".format(T, max_B)), T, max_B)
